#!/usr/bin/env python3
"""
Create CLEAN failure case visualizations - WITH PADDING FOR TEXT BOXES
"""
...
def create_failure_figure(task_name, failure_cases, output_path):
    """Create CLEAN figure - with padding around text boxes"""
    ...
    # Create figure with WHITE BACKGROUND and padding
    ...
    # Create main grid - MORE left margin to prevent text box cutoff
    gs = gridspec.GridSpec(num_models, 15,
                          hspace=0.3, wspace=0.2,
                          left=0.12, right=0.98,  # More left space
    ...
    for model_idx, (model_name, failure_data) in enumerate(models_with_failures):
        ...
        # Model name - positioned with PADDING from edge
        model_display = format_model_name(model_name)
        y_pos = (num_models - model_idx - 0.5) / num_models * 0.92 + 0.04
        
        # Clean text box - positioned at 0.05 with extra padding
    ...
    # Save with NO bbox_inches='tight' to preserve padding
...
def main():
    ...
    print("\n" + "=" * 60)
    print("Creating failure figures with padding...")
    print("=" * 60)
