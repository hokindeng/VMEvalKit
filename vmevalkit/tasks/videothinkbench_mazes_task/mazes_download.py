#!/usr/bin/env python3
"""
VideoThinkBench Mazes Task for VMEvalKit

Downloads Mazes tasks from HuggingFace.
...
def create_dataset(num_samples: int = None) -> Dict[str, Any]:
    """
    Download Mazes dataset from HuggingFace.
    ...
    print(f"📥 Downloading Mazes tasks from HuggingFace...")
    
    dataset = load_dataset('OpenMOSS-Team/VideoThinkBench', 'Mazes', split='test')
    
    pairs = []
    for idx, item in enumerate(dataset):
        task_id = f"mazes_{idx:04d}"
    ...
    print(f"   ✅ Downloaded {len(pairs)} Mazes tasks")
