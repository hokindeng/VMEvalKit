#!/usr/bin/env python3
"""
Create CLEAN failure case visualizations - MODEL NAMES ADJUSTED RIGHT
"""
...
def create_failure_figure(task_name, failure_cases, output_path):
    """Create CLEAN figure - model names adjusted to the right"""
    ...
    for model_idx, (model_name, failure_data) in enumerate(models_with_failures):
        ...
        # Model name - MOVED TO THE RIGHT (0.04 instead of 0.015)
        model_display = format_model_name(model_name)
        y_pos = (num_models - model_idx - 0.5) / num_models * 0.92 + 0.04
        
        # Clean text box with generous padding
        fig.text(0.04, y_pos, model_display,
...
def main():
    ...
    print("\n" + "=" * 60)
    print("Creating failure figures (names adjusted right)...")
    print("=" * 60)
